...
def main():
    ...
    for layer_size in range(3):
        for activation in range(3):
            for neuron in range (3):
                for lrr in range(6): #learning rate
                    model = MyModel(neuron_size[neuron])
                    
                    model = model.to(device)
                    model_number = model_number + 1
                    optimizer = torch.optim.Adam(model.parameters(),lr=learning_rate[lrr]) #learning rate
                    train(model, optimizer, train_dataloader, validation_dataloader, epochs, device,model_number,activation,layer_size)
